            ...
            if (np.random.uniform(0,1) < 0.2):
                ...
                if (door_status):
                    if (np.array([7,1]) == otherfood_pos).all(1).any():
                         otherfood_pos_without_room = [otherfood_pos[0]]
                    else:
    ...
    def train(self):
        ...
        for i in range(self.params['batch_size']):
            ...
            if (not self.sympathetic_mode):
                ...
                #input_RobotNext_full.append(input_RobotNext)
                #input_HumanNext_full.append(input_HumanNext)
...
for episodes in range(agents.params['no_episodes']):
    ...
    #agents.train_human_continue = True
        ...
        for agent_no in [0,1]:
            ...
            if not stop_human_training:
                if (agent_no == 1) and (lastH_state is not None):
                    # Save state, action, reward and next state for robot
                    reward_vector = agents.reward_features(lastH_food[0], lastH_food[1], env.agents[0].foodpos, env.agents[1].foodpos, env.agents[0].door_opened, lastR_door)
            ...
            #print("select action", time.time() - t)
