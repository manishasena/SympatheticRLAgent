...
from .objects import WorldObj, Wall, Goal, Lava, GridAgent, BonusTile, BulkObj, COLORS,Food,FoodOther, EmptySpace
...
class MultiGridEnv(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        for agent in self.agents:
            ...
            if agent.spawn_delay == 0:
                ...
                self.try_place_obj(agent,agent_positions[n])
                agent.activate()
        del n
    ...
    def gen_obs_grid(self, agent):
        ...
        #)
        grid = self.grid.slice(
            topX, topY, agent.view_size, agent.view_size, rot_k=0
        )
    ...
    def step(self, actions):
            ...
            if agent.active and action != 6:
                ...
                # Move forward
                    ...
                    if can_move:
                        ...
                        #
                        if isinstance(fwd_cell, (Food)) and (agent_no == 0):
                            agent.eatfood = True
                            agent.fwd_cell_pos = fwd_cell

                        if isinstance(fwd_cell, (FoodOther)) and (agent_no == 1):
                            agent.eatfood = True
                            agent.fwd_cell_pos = fwd_cell
                ...
                # Toggle/activate an object
                    ...
                    if fwd_cell:
                        ...
                        if fwd_cell.state == 0:
                            agent.door_opened = True
                        if fwd_cell.state == 1:
                            agent.door_opened = True
                        if fwd_cell.state == 2:
                            agent.door_opened = False
                    else:
                        pass
        ...
        for agent in self.agents:
            ...
            agent_n += 1
            step_rewards[agent_n] += -1 #Step penalty
            ...
            if agent.eatfood:
                # Check if other agent is also in square
                if agent_n == 0:
                    if (self.agents[1].pos == agent.fwd_cell_pos.pos).all():
                        self.grid.set(agent.fwd_cell_pos.pos[0],agent.fwd_cell_pos.pos[1], None)
                        agent.foodpos = [ x for x in agent.foodpos if not (x==agent.fwd_cell_pos.pos).all()]
                        self.place_back_agent_obj(agent, **self.agent_spawn_kwargs)
                        agent.eatfood = False
                        self.place_back_agent_obj(self.agents[1], **self.agent_spawn_kwargs)
                    ...
                    if (self.agents[0].pos == agent.fwd_cell_pos.pos).all():
                        self.grid.set(agent.fwd_cell_pos.pos[0],agent.fwd_cell_pos.pos[1], None)
                        agent.foodpos = [ x for x in agent.foodpos if not (x==agent.fwd_cell_pos.pos).all()]
                        self.place_back_agent_obj(agent, **self.agent_spawn_kwargs)
                        agent.eatfood = False
                        self.place_back_agent_obj(self.agents[0], **self.agent_spawn_kwargs)
                    else:
                        self.grid.set(agent.fwd_cell_pos.pos[0],agent.fwd_cell_pos.pos[1], None)
                        agent.foodpos = [ x for x in agent.foodpos if not (x==agent.fwd_cell_pos.pos).all()]
                        self.place_back_agent_obj(agent, **self.agent_spawn_kwargs)
    ...
    def simulatestep(self):
        ...
        iter_agents = list(enumerate(zip(self.agents)))
        iter_order = np.arange(len(iter_agents))
        self.np_random.shuffle(iter_order)
        for shuffled_ix in iter_order:
            agent_no, (agent) = iter_agents[shuffled_ix]
            agent = agent[0]
    ...
    def place_back_agent_obj(self, obj, top=(0,0), size=None, reject_fn=None, max_tries=1e5):
        max_tries = int(max(1, min(max_tries, 1e5)))
        top = (max(top[0], 0), max(top[1], 0))
        if size is None:
            size = (self.grid.width, self.grid.height)
        ...
        # agent_positions = [tuple(agent.pos) if agent.pos is not None else None for agent in self.agents]
        for try_no in range(max_tries):
            pos = obj.pos #self.np_random.randint(top, bottom)
            if (reject_fn is not None) and reject_fn(pos):
                continue
            else:
                if self.try_place_obj(obj, pos):
